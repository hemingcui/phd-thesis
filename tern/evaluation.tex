...
%% compiled programs down to machine instructions using \vv{llvm-gcc -O2}
...
\caption{\small {\em Statistics of programs evaluated.} {\bf Size}
  ...
  annotations (\vv{begin\_task()} and \v{end\_task()}) we inserted.  {\bf
...
return of \vv{cache\_find()} as symbolic (\S\ref{sec:window}).  For MySQL,
...
%% For \pbzip, we randomly selected 10,000 files from \vv{/usr} directory on
...
%% \vv{symbolic()} call. Our schedule cache used little memory.
...
both call \vv{pthread\_mutex\_lock()} and \v{pthread\_mutex\_unlock()} in a
tight loop.
...
%% \pbzip and barnes have been speed up significantly, since we have removed \vv{usleep()} and \v{pthread\_barrier\_wait()} from 
...
%% In Volrend, the \vv{pthread\_barrier\_wait()} operation has also been removed, but it still has a positive overhead (18.5\%).
...
%% in the \vv{before\_landmark()} and \v{after\_landmark()} functions inserted to each synchronization operation, removing synchronization function
%% calls from source code become possible. We have tried to removed \vv{usleep()} in the file writer thread from \pbzip and \v{pthread\_barrier\_wait()} from barnes, volrend and water-nsquared,
%% and the speed up is significant. Please note that \vv{usleep()} in \pbzip is supposed to reduce contention to acquire pthread mutexes,
...
%% reduces contention, so removing \vv{usleep()} can speed up \pbzip. We leave the systermatic investigation of removing synchronization operations in \tern to future work.
...
Table~\ref{table:races}.  We then manually inserted \vv{usleep()} to these
...
%% have to manually insert 2-4 \vv{sleep(0)} calls in the corresponding
%% program to expose the error.  We let \tern memoize these buggy schedules.
%% We then remove the bogus \vv{sleep(0)} calls we insert from the programs.
%% We also remove the \vv{before\_blocking()} and \v{after\_blocking()} calls
%% due to the bogus \vv{sleep(0)} calls from the memoized schedules.  We rerun
