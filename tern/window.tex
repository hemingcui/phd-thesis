...
%% \vv{symbolic()}.  Doing so is often straightforward.  For instance, we
%% inserted only one \vv{symbolic()} to expose the return of Apache's
%% \vv{cache\_find()} function.

%% Moreover, \tern has a mechanism to tolerate missed \vv{symbolic()}
...
%% suppose a \vv{symbolic()} had not been missed, \tern would have memoized
...
%% window, the data marked as symbolic (by \vv{symbolic()} calls) may flow
...
%% constraints on demand by calling \vv{symbolic()} at program points where
%% the program's internal state is observed.  Locating these program points
%% tend to be easy.  For instance, we insert only one such \vv{symbolic()}
%% call for Apache by marking the return of \vv{cache\_find()} symbolic.
...
%% Specifically, suppose if a \vv{symbolic()} had not been missed, \tern would
...
%% %% request processing using \vv{begin\_task()} and \v{end\_task()}.  Doing
%% %% so is typically easy: place \vv{begin\_task(}) after an \v{accept()} of
%% %% client connection and \vv{end\_task()} sends back the reply.  Developers
...
%% %% data marked by \vv{symbolic()} may flow into a server program's internal
...
%% %% \tern avoids symbolic creep as follows.  In \vv{end\_task()}, it concretizes
...
%% %% calling \vv{symbolic()} at program points where the program's internal
...
%% %% % by calling \vv{symbolic()}.
%% %% %  so that \tern considers the return when deciding a schedule.  
%% %% Locating such program points is typically easy.  For
%% %% instance, we insert only one such \vv{symbolic()} call for Apache (making
%% %% the return of \vv{cache\_find()} symbolic).

%% %% %  and XXX such calls for MySQL's key and table
%% %% % caches (returns of \vv{what functions you annotate}).
