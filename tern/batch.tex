\section{Schedule Memoization}  \label{sec:batch}
...
\begin{figure}[t]
\tiny \lgrindfile{tern/code/memoize.cpp.lineno}
...
%% unlock with the most involved one being \v{pthread\_cond\_wait()}.
%% %  and \v{pthread\_create()}.  
%% For brevity, we omit the details and only note that we replace the mutex
%% used in a \v{pthread\_cond\_wait()} call with our own mutex to avoid
