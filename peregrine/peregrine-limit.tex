...
the values returned by \vv{random()}-variants as inputs.  \peregrine ensures that
...
query physical time (\eg, \vv{gettimeofday()}), pointer addresses returned
by \vv{malloc()}, and nondeterminism in the kernel or external libraries,
...
\vv{Libc} implementation.)  Currently \peregrine works only with a single process,
