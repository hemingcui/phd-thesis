...
%% If we change the example slightly and allocate a buffer using \v{malloc()}
%% within \v{SlaveStart()}, \bddbddb also reports that different buffers
...
%% Once we fix the schedule to contain a single \v{lock()} operation,
%% pointers \v{p} and \v{q} can never alias each other with respect to this
...
%% a \v{if}-statement ``\v{if(x) lock(); else ...}'' with the \v{lock()}
%% operation in the schedule, \peregrine removes the false branch of this
%% \v{if}-statement so that alias analysis has one fewer chance to get
%% confused.
