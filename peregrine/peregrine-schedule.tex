...
%\peregrine
considers 28 \vv{pthread} operations, such as \v{pthread\_mutex\_lock()}
...
%% enforcing the sync-schedule between unrelated operations on lock \vv{l1}
%% and \vv{l2}, we effectively make region $T_{1,1}$ happens-before $T_{2,2}$
...
%% These branch instructions include LLVM instructions \vv{br} (direct conditional
%% or unconditional branch), \vv{switch}, \v{indirectbr}, \v{call}, and
%% \vv{invoke} (like call instruction, but with a callee that may throw
%% exceptions).  
...
shown in Figure~\ref{fig:slot}, the \vv{slot()} function interprets the
