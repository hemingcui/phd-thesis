...
\caption{Different multithreading approaches. Red stars represent buggy
  ...
  any input data.  DMT (\subref*{fig:dmt}) may map each input to an arbitrary
...
In this section, we start with preliminaries, then describe the key reason that
...
program below has data races on \vv{balance}: \lgrindfile{code/race-bug.cpp}
...
a series of ``needle-in-a-haystack" challenges. For instance, when facing these
excessive schedules, developers' understanding are prone to mistakes, and we
have seen tons of concurrency bug reports sent to the email lists of real-world
software. Various forms of testing tools suffer, too.  Stress testing is the
common method for (indirectly) testing schedules, but it often redundantly tests
the same schedules while missing others. To mitigate redundant testing effort,
recent advanced tools (\eg,~\cite{musuvathi:chess:osdi08}) can systematically
test schedules and have effectively found harmful bugs in real-world programs,
and these tools have included several smartest algorithms
(\eg,~\cite{flanagan:dynamicpo}) to improve schedule coverage. Unfortunately,
despite of these great effort, these tools still can not cover more than a tiny
fraction of all exponentially many schedules, and concurency bugs within an
untested schedule can show up in production runs and lead to critical failures
and vulnerabilities. In a word, these exponentially many schedules is the key
reason that causes multithreaded programs extremely difficult to understand,
...
programs such as \apache, to desktop utilities such as parallel compression
utility \pbzip, to parallel implementations of computation-intensive algorithms
