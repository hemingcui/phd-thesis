...
complementary technique called Dterministic Multithreading (or \dmt) that
...
input data.  \dmt (\subref*{fig:dmt}) may map each input to an arbitrary
schedule, reducing programs' robustness on input perturbations.  \smt
(\subref*{fig:smt} and \subref*{fig:smtn}) reduces the total set of schedules
...
deterministic (\subref*{fig:smt}) or nondeterministic (\subref*{fig:smtn}).}
\vspace{-.2in}
\end{center}
\end{figure*}
...
analyzing, verification, and replication of multithreaded programs. For
instance, when facing
...
algorithms. All these systematic testing, analysis, and verification tools have
effectively found new harmful concurrency bugs in real-world software.
Unfortunately, despite of these great
...
can show up in production runs and lead to critical failures. Due to this poor
coverage, ``replicating execution states"~\cite{paxos:practical}, a critical
fault-tolerance technique, can easily break when running with multithreaded
programs, because concurrency bugs can show up in some replicas and make
execution states across replicas to diverge. Therefore, we argue that the
exponentially many schedules for all inputs is the key reason
...
challenges in understanding, testing, analyzing, verification, and replication
of
...
\para{Replicating execution states.} By running the same \smt runtime on each
replication node, we can enforce the same small set of schedules across replicas
and avoid divergence of execution states. We will describe our system design on
integrating \smt with replication service in Chapter~\ref{sec:rep}.
...
inherent in existing \dmt systems and our concrete evaluation results that
confirms this problem. Due to this problem, when running with \dmt, the
number of possible schedules for all inputs remains enormous, so a multithreaded
program is still extremely difficult to understand, test, analyze, verify, or
replicate.
