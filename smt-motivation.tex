...
(\eg, \pthread) maintain a many-to-many mapping from inputs to schedules, where
...
can show up in production runs and lead to severe failures. Due to this poor
coverage, state machine replication~\cite{paxos:practical}, a critical
fault-tolerance technique, can easily break when running with multithreaded
programs, because buggy schedules may show up in some replicas and cause
program execution states across replicas to diverge. Therefore, we argue that
the exponentially many schedules for all inputs is the key reason
...
that
carefully enforcing schedules can achieve only moderate overhead. Afterall,
...
inputs for multithreaded programs~\cite{ics:oopsla13} by leveraging some
...
\para{State Machine Replication.} By running the same \smt runtime on each
replication node (or replica), we can enforce the same small set of schedules
for a multithreaded program on all replicas and avoid divergence of execution
states across replicas. We will describe our \crane design on integrating
\smt with state machine replication service in Chapter~\ref{sec:crane}.
