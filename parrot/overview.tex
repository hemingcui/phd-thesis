...
This section first compares two prior systems and \xxx using
an example (\S\ref{sec:example}), and then describes \xxx's architecture
(\S\ref{sec:arch}).
...
\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{figures/dthreads_schedule}
\vspace{-.2in}
...
\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{figures/parrot_schedule}
\vspace{-.2in}
\caption{{\em A \xxx schedule with performance
    hints.}}\label{fig:parrot-schedule}
\vspace{-.05in}
\end{figure}

%% In this subsection, we present an example, explain why two prior systems
%% perform poorly on it, and illustrate how \xxx works with it.
...
Running the example with \xxx is easy; users do
\vspace{-2 mm}
\begin{small}
\begin{verbatim}
...
\noindent
During the execution, \xxx intercepts \pthread synchronizations.  Without
the hints at lines 3 and 23, \xxx schedules the synchronizations using round-robin. 
...
sample runs, and \xxx simplifies performance debugging by
...
%  When running the examplewith \xxx on a 400 MB file, we frequently
...
informs \xxx that the program has a coscheduling group involving
\v{nthreads} threads, and line 23 is the starting point of coscheduling.  With
these hints, \xxx switched to the schedule in
...
%% Note that these hints need not be 100\% accurate because \xxx can time out
...
%% (lines XX and XX) to let \xxx switch to a faster schedule that makes
...
%% hints. Line XXX informs \xxx the size of the parallel computation group.
%% Line XXX informs \xxx the beginning point of a computation.  With the
%% hints, \xxx achieved comparable performance ($<$1\% overhead) to
%% nondeterministic execution.

%% To further help developers, \xxx also logs the wait
%% time of each synchronization for developers to identify excessive delays.

\begin{figure}[t]
\centering
\includegraphics[width=.28\textwidth]{figures/architecture}
%arch:
...
%OS schedule
\vspace{-.05in}
\caption{{\em \xxx architecture.}} \label{fig:arch}
\vspace{-.05in}
\end{figure}

\subsection{Architecture} \label{sec:arch}

Figure~\ref{fig:arch} shows \xxx's architecture.  We designed \xxx to be
