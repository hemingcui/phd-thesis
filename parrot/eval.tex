...
We evaluated \xxx on a diverse set of \nprog programs. This set
...
workpile.  To the best of our knowledge, our evaluation of \xxx
...
%% choosing workloads, we have also evaluated \xxx on three different workloads for each 
...
%% by \xxx, and the Intel \vtune tool to inspect serialization caused by our
...
%% \xxx runtime rather than tranditional \pthread barrier, which may corrupt
...
\begin{figure*}[tb]
\centering
\includegraphics[width=\textwidth]{figures/overhead}
\vspace{-.20in}
\caption{{\em \xxx's performance normalized over nondeterministic
...
Figure~\ref{fig:overhead} compares \xxx's performance to nondeterministic
...
%% \xxx. The reason was that \xxx's implementation of condition variable wait
%% uses \xxx's scheduler \v{wait}, which combines spin- and block-waits, saving
...
%% \xxx because \xxx's schedules improved affinity.

\begin{figure*}[tb]
\centering
\includegraphics[width=\textwidth]{figures/hints}
\vspace{-.20in}
\caption{{\em Effects of performance hints.} They reduced \xxx's overhead from \overallnohints to \overallhints.} \label{fig:overhead-hints}
\vspace{-.05in}
\end{figure*}
...
\end{tabular}
\vspace{-.05in}
\caption{{\em \Compute successes and timeouts.}} \label{tab:timeouts}
\end{table}

Figure~\ref{fig:overhead-hints} compares \xxx's performance with and
...
\vspace{-.2in}
\caption{{\em \xxx, \dthreads, and \coredet overhead.}} \label{fig:comparison}
\vspace{-.0in}
\end{figure}
...
\subsection{Scalability and Sensitivity} \label{sec:sensitivity}
...
%% \caption{{\em \xxx's scalability to the number of cores.} Legend lists
...
%% \caption{{\em \xxx's scalability to workloads.}  Legend lists only
...
%% Figure~\ref{fig:scalability} shows \xxx's scalability on our 24-core
...
%% Figure~\ref{fig:workloads} shows \xxx's scalability on three or four
...
%% shown.  To summarize, the performance of \xxx and its hints are robust
%% to core counts, workload scales, and workload types.

We measured \xxx's scalability on our 24-core machine. All programs
...
teardown.  We also measured \xxx's scalability on three or four different
...
5.  The overhead did not vary much.  To summarize, \xxx's performance is
...
We evaluated \xxx's determinism by verifying that it computed the same
schedules given the same input.  For all programs except those with
\nondets, ad hoc synchronizations, and network operations, \xxx is
...
problem.  We also evaluated \xxx's determinism using a modified version of
...
\xxx, and saw 1,000 different results.  With \xxx, it always computed the
same result.
...
%% run both non-det (original) executions and executions with \xxx, both with
...
\subsection{Model Checking Coverage} \label{sec:coverage}

%% We formed an \ecosys by integrating \xxx with \dbug. Within 
%% this \ecosys, \xxx determinsitically schedules all \pthread synchronizations 
...
%% \nondet for \xxx. But forming this \ecosys, we expect that 
%% \xxx should drastically reduce the state space of a program and
%% increase testing coverage for \dbug, and \dbug can focus on
%% exploring the schedules that matter to \xxx. We enabled 
%% dynamic partial order reduction when running both \ecosys and \dbug.
%% We focus on evaluating how well \xxx can improve 
%% testing coverage for \dbug and how effectively \dbug can explore 
%% the schedules that matter to \xxx in \ecosys.
...
or model checking in general, becomes prohibitive. Consequently, \xxx's
...
\xxx can reduce the state space. We checked each program for a maximum of
...
%  These results also demonstrate the flexibility of \xxx: once schedules
%  are checked, \xxx can use the schedules at runtime for better
%  performance.

%{\bf TODO:} Add \ua and discuss \raytrace.

%% results are promising because \xxx can safely ignore these operations in
