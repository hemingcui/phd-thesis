...
head, current thread does busy wait until
...
corresponding server thread in the wait queue, it dequeues the head \paxos 
operation, and put this thread into the head of the run 
...
Even if the wait queue has no thread waiting for \paxos operations, the run 
queue head thread must wait until the \paxos queue is not empty, because the 
arrival of \paxos requests can be random and may cause the blocking operation 
to be scheduled in the run queue in different ``rounds" in different replicas.
