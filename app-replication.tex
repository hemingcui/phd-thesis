...
running a server program, and SMR ensures 
consensus on the same totally ordered sequence on inputs among replicas, then
active replicas performs the same sequence of state transitions and produce the same sequence of outputs.
...
replica to be only one~\cite{paxos, pbft:osdi99, mencius:osdi08, saebft:sosp03},
or require developers to manually annotate all shared states 
among threads~\cite{eve:osdi12}, which may be time consuming and error prone.

%% P3
To address the above state divergence chllange, one may consider recent
...
or deterministic replay~\cite{rex:eurosys14} can be simply applied to SMR,
...
guaranteeing that different runs on the same or even simplar inputs (enabled by 
\smt) are running the same schedules.
However, although these multi-threading techniques are promising, as we will discuss further in the following sections, 
...
fundamentally preventing these threading systems being applied to SMR (\eg, 
deterministic replay may fail because different replicas may serve very different read-only 
...
transparently replicated multi-threaded servers with two novel techniques.
...
%% P7
\msmr is easy to deploy and has no fundamental assumptions on concurrency.
